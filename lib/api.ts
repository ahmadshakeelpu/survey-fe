...
const BASE_URL = process.env.NEXT_PUBLIC_API_BASE || 'http://localhost:3001/api'
...
export const api = {
  createParticipant: async (payload: { consent_at: string; demographic: DemographicData }) => {
    const response = await axios.post(`${BASE_URL}/participant`, payload)
    return response.data
  },

  saveScales: async (payload: { participant_id: string; attari: AttariData; tai: TaiData }) => {
    const response = await axios.post(`${BASE_URL}/scales`, payload)
    return response.data
  },
  ...
  getGreeting: async (payload: { participant_id: string; round: number }) => {
    const response = await axios.post(`${BASE_URL}/chat/greeting`, payload)
    return response.data
  },

  complete: async (payload: { participant_id: string; post_use: number; post_change: string }) => {
    const response = await axios.post(`${BASE_URL}/complete`, payload)
    return response.data
  }
}
